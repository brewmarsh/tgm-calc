...
def analyze_screenshot(filepath):
    ...
    try:
        ...
        enforcers_match = re.findall(r'[A-Z][a-z]+', text)
        if enforcers_match:
            # Filter out common words that are not enforcer names
            enforcer_names = [name for name in enforcers_match if name not in ['My', 'Enforcers', 'Enforcer', 'Items']]
            if enforcer_names:
                extracted_data['enforcers'] = enforcer_names
